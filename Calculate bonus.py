...
ID of the employee is an odd number and the employee's name does not start with the character 'M'. 
...
def calculate_special_bonus(employees: pd.DataFrame) -> pd.DataFrame:
    ...
    # Apply the bonus calculation function to each row
    employees['bonus'] = employees.apply(calculate_bonus, axis=1)
    
    # Select the 'employee_id' and 'bonus' columns and order by 'employee_id'
